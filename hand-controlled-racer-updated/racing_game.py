...
# --------------------------- DATABASE (players) ----------------------------
# SQLite file sits next to the script. One row per real person: `contact`
# is UNIQUE, so logging in again with the same phone number never creates
# a duplicate row — it just updates `last_login_at` and bumps `login_count`
# on the existing row.
...
def save_or_update_player(name, contact):
    """Insert a new player row, or if this contact number already exists
    (same person logging in again), just touch last_login_at / login_count
    instead of storing their details a second time."""
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    conn = sqlite3.connect(DB_PATH)
    try:
        cur = conn.execute("SELECT id, name FROM players WHERE contact = ?", (contact,))
        row = cur.fetchone()
        if row is None:
            conn.execute(
                "INSERT INTO players (name, contact, first_login_at, last_login_at, login_count) "
                "VALUES (?, ?, ?, ?, 1)",
                (name, contact, now, now),
            )
        else:
            # Same contact number seen before -> this is a returning player.
            # Don't insert a duplicate row; just record the new login.
            conn.execute(
                "UPDATE players SET last_login_at = ?, login_count = login_count + 1, name = ? "
                "WHERE contact = ?",
                (now, name, contact),
            )
        conn.commit()
...
class HandTrackerThread(threading.Thread):
    ...
    def run(self):
        ...
        if not cap.isOpened():
            self.state.update(camera_ok=False, error="Cannot open camera.")
            return
...
class TextInputBox:
    """A minimal clickable/typeable text field used on the login screen."""
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.key == pygame.K_BACKSPACE:
                ...
                pass  # handled by the screen (moves focus / submits)
            elif len(self.text) < self.max_len:
                ch = event.unicode
                if not ch:
                    return
                if self.numeric:
                    # allow digits and a leading '+' for country codes
                    if ch.isdigit() or (ch == "+" and self.text == ""):
                        self.text += ch
                elif ch.isprintable():
                    self.text += ch

    def draw(self, surf, font):
        bg = (255, 255, 255) if self.active else (235, 235, 235)
        pygame.draw.rect(surf, bg, self.rect, border_radius=8)
        border = (60, 200, 255) if self.active else (150, 150, 155)
        pygame.draw.rect(surf, border, self.rect, width=2, border_radius=8)

        if self.text:
            txt_surf = font.render(self.text, True, (25, 25, 30))
        else:
            txt_surf = font.render(self.placeholder, True, (150, 150, 155))
        surf.blit(txt_surf, (self.rect.x + 12, self.rect.y + (self.rect.h - txt_surf.get_height()) // 2))


def run_login_screen(window):
    """Shows a Name / Contact Number form with a Start Game button.
    Blocks until the player submits valid details, then returns
    {"name": ..., "contact": ...}. Returns None if the window was closed
    (in which case the caller should exit)."""
    pygame.display.set_caption("Hand-Controlled Racer — Login")
    win_w, win_h = window.get_size()

    form_w = 480
    cx = win_w // 2
    name_box = TextInputBox((cx - form_w // 2, win_h // 2 - 70, form_w, 52), placeholder="Full name")
    contact_box = TextInputBox(
        (cx - form_w // 2, win_h // 2 + 10, form_w, 52), placeholder="Contact number", numeric=True, max_len=15
    )
    name_box.active = True

    start_btn = pygame.Rect(cx - form_w // 2, win_h // 2 + 100, form_w, 56)

    title_font = pygame.font.SysFont("arial", 44, bold=True)
    present_font = pygame.font.SysFont("arial", 24, bold=True) 
    label_font = pygame.font.SysFont("arial", 20, bold=True)
    field_font = pygame.font.SysFont("arial", 24)
    btn_font = pygame.font.SysFont("arial", 26, bold=True)
    error_font = pygame.font.SysFont("arial", 18)


    logo = pygame.image.load("iete_logo.png").convert_alpha()
    logo = pygame.transform.smoothscale(logo, (120, 120))  # Change size if needed
    ...
    def validate():
        ...
        contact = contact_box.text.strip()
        if not name:
            return None, "Please enter your name."
        if len(re.sub(r"\D", "", contact)) < 7:
            return None, "Please enter a valid contact number (min 7 digits)."
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            contact_box.handle_event(event)

            if event.type == pygame.KEYDOWN and event.key == pygame.K_TAB:
                name_box.active, contact_box.active = contact_box.active, name_box.active

            if event.type == pygame.KEYDOWN and event.key in (pygame.K_RETURN, pygame.K_KP_ENTER):
                if name_box.active:
                    name_box.active, contact_box.active = False, True
                else:
        ...
        window.fill((20, 20, 26))
        window.blit(logo, logo.get_rect(center=(cx, win_h // 2 - 290)))
       # IETE text
        present = present_font.render(
            "IETE Students' Forum Presents",
            True,
            (255, 200, 60)   # Golden color
        )
        window.blit(present, present.get_rect(center=(cx, win_h // 2 - 210)))

        # Game title
        title = title_font.render(
            "Hand-Controlled Racer",
            True,
            (245, 245, 245)
        )
        window.blit(title, title.get_rect(center=(cx, win_h // 2 - 165)))
        ...
        window.blit(sub, sub.get_rect(center=(cx, win_h // 2 - 120)))

        name_lbl = label_font.render("Name", True, (200, 200, 205))
        window.blit(name_lbl, (name_box.rect.x, name_box.rect.y - 24))
        name_box.draw(window, field_font)

        contact_lbl = label_font.render("Contact Number", True, (200, 200, 205))
        window.blit(contact_lbl, (contact_box.rect.x, contact_box.rect.y - 24))
        contact_box.draw(window, field_font)

        hover = start_btn.collidepoint(mouse_pos)
        pygame.draw.rect(window, (80, 230, 120) if hover else (60, 200, 255), start_btn, border_radius=10)
        btn_txt = btn_font.render("Start Game", True, (15, 25, 20))
        window.blit(btn_txt, btn_txt.get_rect(center=start_btn.center))

        if error:
            err_surf = error_font.render(error, True, (255, 110, 110))
            window.blit(err_surf, err_surf.get_rect(center=(cx, start_btn.bottom + 26)))

        hint = error_font.render("Starting the game will turn on your webcam for hand-tracking controls.", True, (150, 150, 155))
        window.blit(hint, hint.get_rect(center=(cx, start_btn.bottom + 56)))
...
class Game:
    def __init__(self, hand_state: HandState, window):
        pygame.display.set_caption("Hand-Controlled Racer")

        # The fullscreen window is created once in main() and reused across
        # login -> game -> restart -> login cycles instead of being
        # recreated each time.
        self.window = window
    ...
    def _window_to_screen_pos(self, pos):
        """Convert a mouse position in real window coordinates to the
        internal fixed-size (WIDTH x HEIGHT) game-surface coordinates the
        HUD/buttons are drawn in, undoing the letterbox scale-up. Returns
        None if the position is outside the scaled game area (e.g. in the
        black letterbox bars)."""
        px, py = pos
        ox, oy = self.scaled_pos
        sw, sh = self.scaled_size
        if not (ox <= px < ox + sw and oy <= py < oy + sh):
            return None
        scale = sw / WIDTH
    ...
    def draw_hud(self):
        ...
        if not self._cam_ok:
            status = f"CAMERA ERROR: {self._cam_error or 'unknown'}"
        ...
        if self.game_over:
            ...
            sub = self.font.render(f"Distance: {int(self.distance)} m", True, CLR_TEXT)
            self.screen.blit(sub, sub.get_rect(center=(WIDTH / 2, HEIGHT / 2 - 2)))

            # Restart button (in game-surface coordinates; converted to real
            # window coordinates for click detection in run()).
            btn_w, btn_h = 200, 50
            self.restart_button_rect = pygame.Rect(
                (WIDTH - btn_w) / 2, HEIGHT / 2 + 30, btn_w, btn_h
            )
            mouse_screen = self._window_to_screen_pos(pygame.mouse.get_pos())
            hover = mouse_screen is not None and self.restart_button_rect.collidepoint(mouse_screen)
            pygame.draw.rect(
                self.screen, (80, 230, 120) if hover else CLR_ACCENT, self.restart_button_rect, border_radius=10
            )
            btn_txt = self.font.render("Restart", True, (15, 25, 20))
            self.screen.blit(btn_txt, btn_txt.get_rect(center=self.restart_button_rect.center))
        else:
            self.restart_button_rect = None
    ...
    def run(self):
        """Runs the game loop until the player quits the app entirely
        ('quit') or clicks Restart after crashing ('restart', which sends
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                    return "quit"
                if event.type == pygame.MOUSEBUTTONDOWN and self.game_over and self.restart_button_rect:
                    screen_pos = self._window_to_screen_pos(event.pos)
                    if screen_pos is not None and self.restart_button_rect.collidepoint(screen_pos):
                        return "restart"
...
def main():
    ...
    # One real fullscreen window, reused across login -> game -> restart
    # -> login cycles (recreating it each time flickers/can lose focus).
    window = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)

    while True:
        player = run_login_screen(window)
        if player is None:
            break  # window closed / Esc on the login screen

        save_or_update_player(player["name"], player["contact"])

        # Webcam turns on the instant Start is pressed.
        hand_state = HandState()
        tracker = HandTrackerThread(hand_state)
        tracker.start()

        game = Game(hand_state, window)
        try:
            action = game.run()
        finally:
            tracker.stop()
        ...
        if action == "quit":
            break
        # action == "restart" -> loop back to the login screen
